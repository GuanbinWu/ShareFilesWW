...
class SingleRootTree {
    ...
    childrenMap() {
        const childrenMap:Map<Node,Array<Node>> = new Map();
        for (const [child, parent] of this.nodes) {
            if (child.item.id===0)continue; 
...
function getTextWidth(text:string, fontSize:number, fontFamily = 'Arial') {
...
export function renderTree(tree:SingleRootTree,disabledIDs:number[],operatingIds:number[],intervalX=16,intervalY=8,paddingX=40,paddingY=40){
    // console.log(tree);
    let assignedtree = assignXY(tree,intervalX,intervalY,paddingX,paddingY,18,30)!;
    // console.log(assignedtree);
    if (!assignedtree){return;}
...
class CleanLines{
    //assert : only vertical or horizental lines
    lines:Line[];
    constructor() {
        this.lines=[]
    }

    is_marked(source:Line,target:Line){
        //具有相同的起点
...
function renderNode(node:Node,is_diabled:boolean,is_opterating:boolean) {
    ...
    btn.className = "tree-node";}
        ...
        if (is_diabled){btn.className="tree-node-ban";}
        if (is_opterating){btn.className="tree-node-operating";}
    }
...
function renderLines(tree:SingleRootTree){
...
function assignXY(tree:SingleRootTree,intervalX:number,intervalY:number,offsetX:number,offsetY:number,width_per_char:number,height:number){
    let root= tree.root();
    if (!root){return;}
    root.x = offsetX;
    root.y = offsetY;
    ...
    function assignMaxHeight(node:Node) {
        if (!childrenMap){return;}
    ...
    function assignChildren(node:Node, x:number, y:number) {        
        node.x = x;
        node.y = y;
...
export function itemsToTree(folders:Array<API.FolderEntry>){
    ...
    const nodePool = new Map<number, Node>();
    const parentMap = new Map<Node, Node>();
    function getNode(entry: API.FolderEntry) {
