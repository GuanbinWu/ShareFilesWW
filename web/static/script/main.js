...
const appState = {
    ...
    allLevel: new Map(),
    allUser: new Map(),
    user: API.emptyType.user,
    token: localStorage.getItem("token") ?? "",
    state: "idle",
    //进入repo时写入
    currentRepo: API.emptyType.repo,
    allFolder: new Map(),
    currentLevel: API.emptyType.lv,
    //进入folder时写入
    currentFolder: API.emptyType.folder,
    allFile: new Map(),
    update_token: function (token) {
        localStorage.setItem("token", token);
        this.token = token;
    },
    get_parent_dir: function (dir) {
    ...
    updateFolderMap: function (items) {
        const tmp = new Map();
    ...
    updateAccountMap: function (items) {
        const tmp = new Map();
        for (const item of items) {
            tmp.set(item.id, item);
        }
    ...
    query_level: function (repo_entry) {
    ...
    init: function (allRepo, allLevel, allUser) {
    ...
    into_idle: async function () {
        ...
        const repos = await API.repo_list(appState.token);
        const levels = await API.level_all(appState.token);
        this.init(repos, levels, users);
        this.currentRepo = API.emptyType.repo;
        this.allFolder = new Map();
        this.currentLevel = API.emptyType.lv;
        this.currentFolder = API.emptyType.folder;
        this.allFile = new Map();
    },
    into_repo: function (repo_entry, allFolders) {
        this.state = "repo";
        this.currentRepo = repo_entry;
        this.updateFolderMap(allFolders);
        this.currentLevel = this.query_level(repo_entry);
        this.currentFolder = API.emptyType.folder;
        this.allFile = new Map();
    },
    into_folder: function (folder_entry, allFiles) {
        this.state = "folder";
        this.currentFolder = folder_entry;
...
async function rcycState() {
    ...
    fileContainer.addEventListener("contextmenu", async (e) => {
        ...
        if (name === undefined || creator === undefined)
            return;
        if (creator != appState.user.username)
            return;
...
function toolBar(refreshfn, cwd, cb_open, cb_close, cb_cked) {
...
async function folderState(repo_entry, folder_entry, permission) {
    ...
    const enable_operate = (file_entry) => {
        if (permission === "limit" && appState.user.username != file_entry.creator)
            return false;
        return true;
    };
...
async function folderMenu(event, repo_entry, folder_entry, repo_pub, mylevel, permission) {
...
async function fileMenu(event, repo_entry, folder_entry, file_entry, enable_edit, refreshfn) {
    const existing = document.querySelector('.context-menu');
    if (existing) {
        existing.remove();
    }
...
async function rcycMenu(event, id, folder, repo, name, refreshfn) {
    const existing = document.querySelector('.context-menu');
    if (existing) {
        existing.remove();
    }
...
async function download_rcyc_file(id, name) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement("div");
    box.className = "modal-box";
...
async function restore_rcyc_file(id, folder, repo, refreshfn) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement("div");
    box.className = "modal-box";
...
//done
async function edit_folder(repo_entry, folder, max_level) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement('div');
    box.className = 'modal-box';
    ...
    const confirmfn = async () => {
        try {
            ...
            if (new_level != folder.level || new_name != folder.name) {
                const new_folder = { id: folder.id, name: new_name, parent_id: folder.parent_id, repo: folder.repo, level: new_level };
...
//done
async function move_folder(repo_entry, folder_entry, my_level) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement('div');
    box.className = 'modal-box';
    ...
    const confirmfn = async () => {
        try {
            ...
            const new_folder = { id: folder_entry.id, name: folder_entry.name, level: folder_entry.level, parent_id: Number(slct.dataset.id), repo: folder_entry.repo };
            const res = await API.folder_edit(new_folder, appState.token);
            err.set("Ok");
            await Utils.sleep(1000);
            overlay.remove();
...
async function create_folder(repo_entry, folder_entry, max_level) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement('div');
    box.className = 'modal-box';
    ...
    const confirmfn = async () => {
        try {
            ...
            const f = { id: 0, name: new_name, parent_id: folder_entry.id, repo: repo_entry.id, level: new_level };
...
async function del_folder(repo_entry, folder_entry) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement('div');
    box.className = 'modal-box';
...
async function empty_folder(repo_entry, folder_entry) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement('div');
    box.className = 'modal-box';
...
//todo!
async function download_folder_flat(repo_entry, folder_entry) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement("div");
    box.className = "modal-box";
...
async function upload_files(repo_entry, folder_entry, refreshfn) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement("div");
    box.className = "modal-box";
...
async function detail_file(repo_entry, folder_entry, file_entry) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement('div');
    box.className = 'modal-box';
...
//todo
async function copy_file(repo, folder, file, refreshfn) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement('div');
    box.className = 'modal-box';
...
async function move_file(repo, folder, file, refreshfn) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement('div');
    box.className = 'modal-box';
...
async function edit_file(repo, folder, file, enable_edit, refreshfn) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement('div');
    box.className = 'modal-box';
...
async function del_file(repo, folder, file, refreshfn) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement('div');
    box.className = 'modal-box';
...
async function create_repo() {
    const repo_entry = { id: 0, name: "", public: false };
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement('div');
    box.className = 'modal-box';
    box.innerHTML = ``;
    const checkfn = (chekced) => {
        if (chekced) {
...
async function edit_repo(repo_entry, is_admin) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement('div');
    box.className = 'modal-box';
    ...
    const confirmfn = async () => {
        try {
            ...
            const newRepo = { id: repo_entry.id, name: name, public: publicity };
            //edit repo
...
async function del_repo(repo_etntry) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    const box = document.createElement('div');
    box.className = 'modal-box';
...
function levelScroll(init_value, min, max, step) {
...
async function repoMenu(event, repo_entry, is_admin) {
    const existing = document.querySelector('.context-menu');
    if (existing) {
        existing.remove();
    }
...
//   });
// }
function levelEntry(min, max, level_entry, enable_edit) {
    const box = document.createElement("div");
    ...
    function setValue(v) {
        if (v < min) {
            level_value.textContent = String(min);
            return;
        }
        if (v > max) {
            level_value.textContent = String(max);
            return;
        }
    ...
    function collect() {
        ...
        const target = { id: level_entry.id, repo: level_entry.repo, userid: level_entry.userid, level: Number(level_value.textContent) };
        return { source: source, target: target };
    }
    return { el: box, get: collect };
}
function publicityEditor(title, publicity, enable_edit, checkfn) {
    const t = document.createElement("div");
    t.className = "modal-title";
    t.textContent = title;
...
function nameEditor(title, placeholder, enable_edit) {
    const box = document.createElement("div");
    const name_title = document.createElement("div");
    name_title.className = "modal-title";
...
function creatorEditor(title, users, enable_edit) {
    const box = document.createElement("div");
    const t = document.createElement("div");
    t.className = "modal-title";
    t.textContent = title;
...
function levelEditor(enable_edit, repo_entry) {
