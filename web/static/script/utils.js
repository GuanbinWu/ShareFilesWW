...
export function levelNotes(level) {
...
export function confirmBtns(confirmName, cancelName, confirmfn, cancelfn) {
...
export function get_permission(repo_public, user_level, folder_level) {
    if (repo_public) {
        return "free";
    }
    else {
        if (user_level < folder_level)
            return "ban";
        if (user_level === folder_level)
            return "limit";
        if (user_level > folder_level)
...
export function toolbar_logo(id, src, title) {
...
export function goBack_floating_window(gobackfn) {
    ...
    icon.title = "返回上一视图";
    icon.addEventListener("click", () => gobackfn());
    div.append(icon);
    return div;
}
export function is_dir_name_valid(name) {
    if (!name) {
...
export function is_file_name_valid(name) {
    if (!name) {
        throw new Error("空名称显然行不通，你在试图操作虚空，这很危险！");
    }
    const hasWhitespace = /\s/.test(name);
